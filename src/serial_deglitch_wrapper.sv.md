...
`serial_deglitch_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `serial_deglitch`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["serial_deglitch_wrapper\nWrapper"]
    WR --> CORE["serial_deglitch\nCore Module"]
```
...
| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `SIZE` | `4` | `parameter int unsigned SIZE = 4` | core 모듈로 동일 전달 |
