...
`exp_backoff_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `exp_backoff`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["exp_backoff_wrapper\nWrapper"]
    WR --> CORE["exp_backoff\nCore Module"]
```
...
| `set_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `clr_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `is_zero_o` | `output` | `logic` | 출력 데이터/상태 신호 |

## 파라미터

| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `Seed` | `'hffff` | `parameter int unsigned Seed = 'hffff` | core 모듈로 동일 전달 |
| `MaxExp` | `16` | `parameter int unsigned MaxExp = 16` | core 모듈로 동일 전달 |
