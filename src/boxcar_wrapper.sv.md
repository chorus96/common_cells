...
`boxcar_wrapper`는 AMD Custom IP Packaging용 패스스루(wrapper) 모듈로, 내부에서 원본 `boxcar` 모듈을 1:1로 인스턴스화합니다. Wrapper 자체의 기능 로직은 없고, 파라미터/포트를 외부에 노출하는 목적입니다.

## 블록 다이어그램

```mermaid
graph LR
    A["External IP Ports"] --> W["boxcar_wrapper\n(pass-through wrapper)"]
    W --> C["boxcar\n(core module)"]
```
...
| 포트명 | 방향 | 타입/폭 | 설명 |
|--------|------|---------|------|
| `lsb_i` | `input` | `idx_t` | 원본 모듈 `boxcar`로 전달되는 포트 |
| `msb_i` | `input` | `idx_t` | 원본 모듈 `boxcar`로 전달되는 포트 |
| `mask_o` | `output` | `mask_t` | 원본 모듈 `boxcar`로 전달되는 포트 |
