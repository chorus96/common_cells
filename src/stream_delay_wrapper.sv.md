...
`stream_delay_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `stream_delay`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["stream_delay_wrapper\nWrapper"]
    WR --> CORE["stream_delay\nCore Module"]
```
...
| `payload_i` | `input` | `logic [payload_t_WIDTH-1:0]` | 입력 데이터/제어 신호 |
| `ready_o` | `output` | `logic` | 핸드셰이크 ready 신호 |
| `valid_i` | `input` | `logic` | 핸드셰이크 valid 신호 |
| `payload_o` | `output` | `logic [payload_t_WIDTH-1:0]` | 출력 데이터/상태 신호 |
...
| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `StallRandom` | `0` | `parameter bit StallRandom = 0` | core 모듈로 동일 전달 |
| `FixedDelay` | `1` | `parameter int FixedDelay = 1` | core 모듈로 동일 전달 |
| `payload_t_WIDTH` | `1` | `parameter int unsigned payload_t_WIDTH = 1` | flat 데이터 폭 설정 파라미터 |
| `Seed` | `'0` | `parameter logic [15:0] Seed = '0` | core 모듈로 동일 전달 |
