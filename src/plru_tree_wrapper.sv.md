...
`plru_tree_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `plru_tree`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["plru_tree_wrapper\nWrapper"]
    WR --> CORE["plru_tree\nCore Module"]
```
...
| `used_i` | `input` | `logic [ENTRIES-1:0]` | 입력 데이터/제어 신호 |
| `plru_o` | `output` | `logic [ENTRIES-1:0]` | 출력 데이터/상태 신호 |

## 파라미터

| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `ENTRIES` | `16` | `parameter int unsigned ENTRIES = 16` | core 모듈로 동일 전달 |
