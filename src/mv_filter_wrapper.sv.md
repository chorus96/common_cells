...
`mv_filter_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `mv_filter`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["mv_filter_wrapper\nWrapper"]
    WR --> CORE["mv_filter\nCore Module"]
```
...
| `sample_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `clear_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `d_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `q_o` | `output` | `logic` | 출력 데이터/상태 신호 |
...
| `THRESHOLD` | `10` | `parameter int unsigned THRESHOLD = 10` | core 모듈로 동일 전달 |
