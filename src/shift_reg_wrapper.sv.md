...
`shift_reg_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `shift_reg`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["shift_reg_wrapper\nWrapper"]
    WR --> CORE["shift_reg\nCore Module"]
```
...
| `d_i` | `input` | `logic [dtype_WIDTH-1:0]` | 입력 데이터/제어 신호 |
| `d_o` | `output` | `logic [dtype_WIDTH-1:0]` | 출력 데이터/상태 신호 |
...
| `Depth` | `1` | `parameter int unsigned Depth = 1` | core 모듈로 동일 전달 |
