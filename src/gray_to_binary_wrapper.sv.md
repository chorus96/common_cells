# gray_to_binary_wrapper (`gray_to_binary_wrapper.sv`)

## 개요

`gray_to_binary_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `gray_to_binary`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["gray_to_binary_wrapper\nWrapper"]
    WR --> CORE["gray_to_binary\nCore Module"]
```
...
| 포트명 | 방향 | 타입/폭 | 신호 설명 |
|---|---|---|---|
| `A` | `input` | `logic [N-1:0]` | core 모듈로 전달되는 신호 |
| `Z` | `output` | `logic [N-1:0]` | core 모듈로 전달되는 신호 |

## 파라미터

| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `N` | `-1` | `parameter int N = -1` | core 모듈로 동일 전달 |
