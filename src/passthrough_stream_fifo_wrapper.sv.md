...
`passthrough_stream_fifo_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `passthrough_stream_fifo`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["passthrough_stream_fifo_wrapper\nWrapper"]
    WR --> CORE["passthrough_stream_fifo\nCore Module"]
```
...
| `SameCycleRW` | `1'b1` | `parameter bit SameCycleRW = 1'b1` | core 모듈로 동일 전달 |
