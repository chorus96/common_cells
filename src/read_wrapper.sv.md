...
`read_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `read`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["read_wrapper\nWrapper"]
    WR --> CORE["read\nCore Module"]
```
...
| 포트명 | 방향 | 타입/폭 | 신호 설명 |
|---|---|---|---|
| `d_i` | `input` | `logic [T_WIDTH-1:0]` | 입력 데이터/제어 신호 |
| `d_o` | `output` | `logic [T_WIDTH-1:0]` | 출력 데이터/상태 신호 |
