...
`lfsr_16bit_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `lfsr_16bit`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["lfsr_16bit_wrapper\nWrapper"]
    WR --> CORE["lfsr_16bit\nCore Module"]
```
...
| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `SEED` | `8'b0` | `parameter logic [15:0] SEED = 8'b0` | core 모듈로 동일 전달 |
| `WIDTH` | `16` | `parameter int unsigned WIDTH = 16` | core 모듈로 동일 전달 |
