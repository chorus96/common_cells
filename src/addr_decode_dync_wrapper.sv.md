...
`addr_decode_dync_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `addr_decode_dync`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["addr_decode_dync_wrapper\nWrapper"]
    WR --> CORE["addr_decode_dync\nCore Module"]
```
...
| `en_default_idx_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `default_idx_i` | `input` | `logic [idx_t_WIDTH-1:0]` | 입력 데이터/제어 신호 |
| `config_ongoing_i` | `input` | `logic` | 입력 데이터/제어 신호 |
...
| `Napot` | `0` | `parameter bit Napot = 0` | core 모듈로 동일 전달 |
| `IdxWidth` | `cf_math_pkg::idx_width(NoIndices)` | `parameter int unsigned IdxWidth = cf_math_pkg::idx_width(NoIndices)` | core 모듈로 동일 전달 |
