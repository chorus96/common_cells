...
`mem_to_banks_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `mem_to_banks`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["mem_to_banks_wrapper\nWrapper"]
    WR --> CORE["mem_to_banks\nCore Module"]
```
...
| `atop_i` | `input` | `logic [atop_t_WIDTH-1:0]` | 입력 데이터/제어 신호 |
...
| `bank_atop_o` | `output` | `logic [atop_t_WIDTH-1:0] [NumBanks-1:0]` | 출력 데이터/상태 신호 |
...
| `AtopWidth` | `32'd0` | `parameter int unsigned AtopWidth = 32'd0` | core 모듈로 동일 전달 |
...
| `atop_t_WIDTH` | `1` | `parameter int unsigned atop_t_WIDTH = 1` | flat 데이터 폭 설정 파라미터 |
