...
`fifo_v3_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `fifo_v3`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["fifo_v3_wrapper\nWrapper"]
    WR --> CORE["fifo_v3\nCore Module"]
```
...
| `data_i` | `input` | `logic [dtype_WIDTH-1:0]` | 입력 데이터/제어 신호 |
| `push_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `data_o` | `output` | `logic [dtype_WIDTH-1:0]` | 출력 데이터/상태 신호 |
| `pop_i` | `input` | `logic` | 입력 데이터/제어 신호 |
...
| `dtype_WIDTH` | `1` | `parameter int unsigned dtype_WIDTH = 1` | flat 데이터 폭 설정 파라미터 |
