...
`lfsr_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `lfsr`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["lfsr_wrapper\nWrapper"]
    WR --> CORE["lfsr\nCore Module"]
```
...
| `out_o` | `output` | `logic [OutWidth-1:0]` | 출력 데이터/상태 신호 |

## 파라미터

| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `LfsrWidth` | `64` | `parameter int unsigned LfsrWidth = 64` | core 모듈로 동일 전달 |
| `OutWidth` | `8` | `parameter int unsigned OutWidth = 8` | core 모듈로 동일 전달 |
| `RstVal` | `'1` | `parameter logic [LfsrWidth-1:0] RstVal = '1` | core 모듈로 동일 전달 |
| `CipherLayers` | `0` | `parameter int unsigned CipherLayers = 0` | core 모듈로 동일 전달 |
| `CipherReg` | `1'b1` | `parameter bit CipherReg = 1'b1` | core 모듈로 동일 전달 |
