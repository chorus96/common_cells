...
`stream_omega_net_wrapper`는 AMD Custom IP Packaging용 패스스루(wrapper) 모듈로, 내부에서 원본 `stream_omega_net` 모듈을 1:1로 인스턴스화합니다. Wrapper 자체의 기능 로직은 없고, 파라미터/포트를 외부에 노출하는 목적입니다.

## 블록 다이어그램

```mermaid
graph LR
    A["External IP Ports"] --> W["stream_omega_net_wrapper\n(pass-through wrapper)"]
    W --> C["stream_omega_net\n(core module)"]
```
...
| 포트명 | 방향 | 타입/폭 | 설명 |
|--------|------|---------|------|
| `clk_i` | `input` | `logic` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `rst_ni` | `input` | `logic` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `flush_i` | `input` | `logic` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `rr_i` | `input` | `logic [idx_inp_t_WIDTH-1:0] [NumOut-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `data_i` | `input` | `logic [payload_t_WIDTH-1:0] [NumInp-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `sel_i` | `input` | `logic [sel_oup_t_WIDTH-1:0] [NumInp-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `valid_i` | `input` | `logic [NumInp-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `ready_o` | `output` | `logic [NumInp-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `data_o` | `output` | `logic [payload_t_WIDTH-1:0] [NumOut-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `idx_o` | `output` | `logic [idx_inp_t_WIDTH-1:0] [NumOut-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `valid_o` | `output` | `logic [NumOut-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
| `ready_i` | `input` | `logic [NumOut-1:0]` | 원본 모듈 `stream_omega_net`로 전달되는 포트 |
...
| `parameter int unsigned Radix       = 32'd2` | `parameter int unsigned Radix       = 32'd2` | Wrapper에서 동일 이름으로 core에 전달 |
...
| `parameter bit          SpillReg = 1'b0` | `parameter bit          SpillReg = 1'b0` | Wrapper에서 동일 이름으로 core에 전달 |
