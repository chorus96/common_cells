...
`binary_to_gray_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `binary_to_gray`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["binary_to_gray_wrapper\nWrapper"]
    WR --> CORE["binary_to_gray\nCore Module"]
```
