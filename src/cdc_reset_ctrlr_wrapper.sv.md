# cdc_reset_ctrlr_wrapper (`cdc_reset_ctrlr_wrapper.sv`)

## 개요

`cdc_reset_ctrlr_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `cdc_reset_ctrlr`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["cdc_reset_ctrlr_wrapper\nWrapper"]
    WR --> CORE["cdc_reset_ctrlr\nCore Module"]
```
