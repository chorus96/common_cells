...
`rstgen_bypass_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `rstgen_bypass`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["rstgen_bypass_wrapper\nWrapper"]
    WR --> CORE["rstgen_bypass\nCore Module"]
```
...
| `rst_test_mode_ni` | `input` | `logic` | 리셋 신호 |
| `test_mode_i` | `input` | `logic` | 입력 데이터/제어 신호 |
| `rst_no` | `output` | `logic` | 리셋 신호 |
| `init_no` | `output` | `logic` | core 모듈로 전달되는 신호 |

## 파라미터

| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `NumRegs` | `4` | `parameter int unsigned NumRegs = 4` | core 모듈로 동일 전달 |
