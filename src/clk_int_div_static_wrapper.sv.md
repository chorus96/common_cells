...
`clk_int_div_static_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `clk_int_div_static`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["clk_int_div_static_wrapper\nWrapper"]
    WR --> CORE["clk_int_div_static\nCore Module"]
```
...
| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `DIV_VALUE` | `1` | `parameter int unsigned DIV_VALUE = 1` | core 모듈로 동일 전달 |
| `ENABLE_CLOCK_IN_RESET` | `1'b1` | `parameter bit ENABLE_CLOCK_IN_RESET = 1'b1` | core 모듈로 동일 전달 |
