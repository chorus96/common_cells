...
`stream_fork_dynamic_wrapper`는 AMD Vivado Custom IP Packager 통합을 위한 패스스루(wrapper)입니다. 내부에 `stream_fork_dynamic`를 1:1로 인스턴스화하며, wrapper 자체는 기능 로직을 추가하지 않습니다.

## 블록 다이어그램
```mermaid
graph LR
    EXT["Vivado IP Integrator / External Ports"] --> WR["stream_fork_dynamic_wrapper\nWrapper"]
    WR --> CORE["stream_fork_dynamic\nCore Module"]
```
...
| `sel_i` | `input` | `logic [N_OUP-1:0]` | 입력 데이터/제어 신호 |
| `sel_valid_i` | `input` | `logic` | 핸드셰이크 valid 신호 |
| `sel_ready_o` | `output` | `logic` | 핸드셰이크 ready 신호 |
| `valid_o` | `output` | `logic [N_OUP-1:0]` | 핸드셰이크 valid 신호 |
| `ready_i` | `input` | `logic [N_OUP-1:0]` | 핸드셰이크 ready 신호 |

## 파라미터

| 파라미터 | 기본값 | 선언 | 설명 |
|---|---|---|---|
| `N_OUP` | `32'd0` | `parameter int unsigned N_OUP = 32'd0` | core 모듈로 동일 전달 |
